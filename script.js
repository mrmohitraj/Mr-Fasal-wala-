...
const DEMO_FASAL_PRODUCTS = [];
const DEMO_WASTE_PRODUCTS = [];
let FASAL_PRODUCTS = [...DEMO_FASAL_PRODUCTS];
let WASTE_PRODUCTS = [...DEMO_WASTE_PRODUCTS];
let ALL_PRODUCTS = [...FASAL_PRODUCTS, ...WASTE_PRODUCTS];

// Called by products-live.js whenever Firestore products change.
// Real (live) products are shown first, demo products fill the rest.
function mergeLiveProducts(liveProducts){
  const liveFasal = liveProducts.filter(p => p.category === "fasal");
  const liveWaste = liveProducts.filter(p => p.category === "waste");
  FASAL_PRODUCTS = [...liveFasal, ...DEMO_FASAL_PRODUCTS];
  WASTE_PRODUCTS = [...liveWaste, ...DEMO_WASTE_PRODUCTS];
  ALL_PRODUCTS = [...FASAL_PRODUCTS, ...WASTE_PRODUCTS];
  renderStaticGrids();
  renderFasalSection();
  renderWasteSection();
}
...
/* ---------------------------------------------------------
   ...
   --------------------------------------------------------- */
function loadCart(){
  try{ return JSON.parse(localStorage.getItem("mfw_cart")) || []; }catch(e){ return []; }
}
function saveCart(){
  try{ localStorage.setItem("mfw_cart", JSON.stringify(cart)); }catch(e){}
}
function loadWishlist(){
  try{ return new Set(JSON.parse(localStorage.getItem("mfw_wishlist")) || []); }catch(e){ return new Set(); }
}
function saveWishlist(){
  try{ localStorage.setItem("mfw_wishlist", JSON.stringify([...wishlist])); }catch(e){}
}
...
function productCardHTML(p){
  ...
  <article class="product-card" data-id="${p.id}">
    <div class="product-card__media">
      ...
      ${p.image ? `<img src="${p.image}" alt="${p.name}" loading="lazy">` : `<span>${p.icon}</span>`}
...
function updateCartUI(){
  ...
  body.innerHTML = cart.map(c=>{
    ...
    <div class="cart-item" data-id="${p.id}">
      <div class="cart-item__media">${p.image ? `<img src="${p.image}" alt="${p.name}">` : p.icon}</div>
...
function openProductModal(id){
  ...
  document.getElementById("productModalContent").innerHTML = `
    <div class="product-modal">
      <div class="product-modal__media">${p.image ? `<img src="${p.image}" alt="${p.name}">` : p.icon}</div>
...
function updateWishlistUI(){
  ...
  if(wishlist.size === 0){
    body.innerHTML = `<div class="wishlist-empty">♥ Your wishlist is empty.<br>Tap the heart icon on any product to save it here.</div>`;
    return;
  }
  body.innerHTML = [...wishlist].map(id=>{
    ...
    <div class="wishlist-item" data-id="${p.id}">
      <div class="wishlist-item__media">${p.image ? `<img src="${p.image}" alt="${p.name}">` : p.icon}</div>
      <div class="wishlist-item__info">
        <div class="wishlist-item__name">${p.name}</div>
        <div class="wishlist-item__price">${money(p.price)} / ${p.unit}</div>
      </div>
      <div class="wishlist-item__actions">
        <button class="wishlist-item__move" data-action="wishlist-move" data-id="${p.id}">Add to Cart</button>
        <button class="wishlist-item__remove" data-action="wishlist-remove" data-id="${p.id}">Remove</button>
      </div>
    </div>`;
  }).join("");
}
...
// Register service worker so the site can be installed as a PWA / converted to APK
...
function sendOrderOnWhatsApp(name, mobile, address, pincode){
  ...
  let lines = [];
  lines.push(`🌾 *New Order — Mr Fasal Wala*`);
  ...
  cart.forEach(c=>{
    const p = getProduct(c.id);
    if(!p) return;
    lines.push(`- ${p.name} x${c.qty} (${p.unit}) — ${money(p.price * c.qty)}`);
  });
  lines.push(``);
  lines.push(`Cart Total: ${money(subtotal)}`);
  lines.push(`Delivery Charge: ${money(delivery)}`);
  lines.push(`*Grand Total: ${money(total)}*`);
  lines.push(``);
  lines.push(`Payment Method: Cash on Delivery (COD)`);

  const message = encodeURIComponent(lines.join("\n"));
  const waUrl = `https://wa.me/${OWNER_WHATSAPP_NUMBER}?text=${message}`;

  window.open(waUrl, "_blank");

  if(window.MFW_saveOrder){
    window.MFW_saveOrder({
      orderNumber: orderId,
      items: cart.map(c => { const p = getProduct(c.id); return p ? {name:p.name, qty:c.qty, price:p.price} : null; }).filter(Boolean),
      subtotal, delivery, total,
      customerName: name, mobile, address, pincode
    });
  }
...
function wireEvents(){
  ...
  document.body.addEventListener("click", (e)=>{
    ...
    else if(action === "wishlist-move"){
      addToCart(id, 1);
      wishlist.delete(id);
      updateWishlistUI();
      document.querySelectorAll(`.product-card__wishlist[data-id="${id}"]`).forEach(b=>b.classList.remove("active"));
    }
  ...
  // Wishlist open/close
  document.getElementById("wishlistBtn").addEventListener("click", ()=>{ updateWishlistUI(); openWishlist(); });
  ...
  document.getElementById("checkoutForm").addEventListener("submit",(e)=>{
    ...
    sendOrderOnWhatsApp(name, mobile, address, pincode);
  });
...
function toggleDarkMode(){
  ...
  localStorage.setItem("mfw_theme", isDark ? "dark" : "light");
}
function initDarkMode(){
  if(localStorage.getItem("mfw_theme") === "dark"){
    document.documentElement.classList.add("dark");
  }
}

/* ---------------------------------------------------------
   17. LANGUAGE TOGGLE (EN / HI)
   --------------------------------------------------------- */
const I18N = {
  "nav-home": {en:"Home", hi:"होम"},
  "nav-fasal": {en:"🌾 Fasal", hi:"🌾 फसल"},
  "nav-waste": {en:"🌿 Fasal Waste", hi:"🌿 फसल कचरा"},
  "nav-deals": {en:"Today's Deals", hi:"आज के सौदे"},
  "nav-mandi": {en:"Market Prices", hi:"मंडी भाव"},
  "nav-schemes": {en:"Govt Schemes", hi:"सरकारी योजनाएं"},
  "nav-contact": {en:"Contact", hi:"संपर्क करें"},
  "nav-sell": {en:"📷 Sell Your Product", hi:"📷 अपनी फसल बेचें"},
  "hero-eyebrow": {en:"🌱 Direct from farm to buyer", hi:"🌱 सीधे खेत से खरीदार तक"},
  "hero-title": {en:"India's Trusted<br><span>Digital Mandi</span>", hi:"भारत की भरोसेमंद<br><span>डिजिटल मंडी</span>"},
  "hero-desc": {en:"Buy quality crops and sell crop waste at fair mandi prices — no middlemen, no login, just fast and honest trade for every farmer and buyer in India.", hi:"अच्छी फसल खरीदें और फसल कचरा सही मंडी भाव पर बेचें — कोई बिचौलिया नहीं, हर किसान और खरीदार के लिए तेज़ और ईमानदार व्यापार।"},
  "hero-buy": {en:"Buy Crops", hi:"फसल खरीदें"},
  "hero-sell": {en:"Sell Crops", hi:"फसल बेचें"},
  "login-btn": {en:"Login", hi:"लॉगिन"}
};
function toggleLanguage(){
  const current = localStorage.getItem("mfw_lang") || "en";
  const next = current === "en" ? "hi" : "en";
  applyLanguage(next);
}
function applyLanguage(lang){
  localStorage.setItem("mfw_lang", lang);
  document.getElementById("langToggleBtn").textContent = lang === "en" ? "हिं" : "EN";
...
function initLanguage(){
  applyLanguage(localStorage.getItem("mfw_lang") || "en");
}
